...
TempApply <- function(df, domain, year){
  ...
  df$MapYear <- as.numeric(df$MapYear)
  df$AVG_YEAR <- as.numeric(df$AVG_YEAR)
  
  #filter +- 10 years older or newer plots to the map year
   df <- subset(df, df$AVG_YEAR < year+11 #############!!!!!!!!!!!!!!!!! only for the filtering
            & df$AVG_YEAR > year-11)
  ...
  #filter eco-region first
   if (domain %in% unique(df$GEZ)==T){
     ...
     below <- subset(df.old, MapYear < year) #non-NAs
     above <- subset(df.old, MapYear > year) #non-NAs
     static <- subset(df.old, is.na(MapYear) | MapYear == year) #NAs MapYear OR 2010 subsets
     ...
     below$AGB_T_HA <- below$AGB_T_HA + (ifelse(below$AGB_T_HA < 100, below$GR3, below$GR2) * (year - below$MapYear))  
     above$AGB_T_HA <- above$AGB_T_HA - (ifelse(above$AGB_T_HA < 100, above$GR3, above$GR2) * (above$MapYear -  year))  
     
     below$AGB_T_HA <- ifelse(below$AGB_T_HA > 152, 
                              below$AGB_ORIG + (below$GR1 * (year - below$MapYear)), below$AGB_T_HA)
     
     above$AGB_T_HA <- ifelse(above$AGB_T_HA > 152, 
                              above$AGB_ORIG - (above$GR1 * (above$MapYear - year)), above$AGB_T_HA) #retain if not in primary/GR3 class 
...
TempVar <- function(df, domain, year){
  ...
  below$AGB_T_HA <- below$AGB_T_HA_ORIG + (ifelse(below$AGB_T_HA_ORIG < 100, below$SD3, below$SD2) * (year - below$MapYear))  
  above$AGB_T_HA <- above$AGB_T_HA_ORIG - (ifelse(above$AGB_T_HA_ORIG < 100, above$SD3, above$SD2) * (above$MapYear -  year))  
  
  below$AGB_T_HA <- ifelse(below$AGB_T_HA > 152, 
                           below$AGB_T_HA_ORIG + (below$SD1 * (year - below$MapYear)), below$AGB_T_HA)
  
  above$AGB_T_HA <- ifelse(above$AGB_T_HA > 152, 
                           above$AGB_T_HA_ORIG - (above$SD1 * (above$MapYear - year)), above$AGB_T_HA) #retain if not in primary/SD3 class 
